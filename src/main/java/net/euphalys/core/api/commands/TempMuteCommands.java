...
public class TempMuteCommands extends AbstractCommands {

    public TempMuteCommands() {
        super("tempmute", "euphalys.cmd.tempmute");
    }

    SanctionsType sanctionsType = SanctionsType.MUTE;

    @Override
    public boolean onCommand(Player player, String[] args) {
        IEuphalysPlayer azoPlayer = api.getPlayer(player.getUniqueId());
        if (!azoPlayer.hasPermission("azonarya.cmd.tempban")) return false;
        if (args.length < 3)
            return false;
    ...
    protected void displayHelp() {
        player.sendMessage("/tempmute <player> <duration> <message>");
    }
}
