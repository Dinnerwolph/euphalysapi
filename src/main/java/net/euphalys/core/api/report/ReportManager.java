...
public class ReportManager implements IReportManager {
    ...
    public void addReport(IEuphalysPlayer target, IEuphalysPlayer player, String message) {
        try {
            ...
            PreparedStatement statement = connection.prepareStatement("INSERT INTO `report`(`euphaId`, `by`, `reason`, `enable`) VALUES (?,?,?,?)");
    ...
    public List<IReport> getReports(IEuphalysPlayer player) {
        ...
        try {
            Connection connection = dataSource.getConnection();
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM `report` WHERE `euphaId`=? AND `enable`=?");
            statement.setInt(1, player.getEuphalysId());
            statement.setInt(2, 1);
            ResultSet resultSet = statement.executeQuery();
            while (resultSet.next()) {
                Report report = new Report(resultSet.getInt("id"), resultSet.getInt("by"), resultSet.getInt("euphaId"), resultSet.getString("reason"));
    ...
    public IReport getReport(int reportId) {
        try {
            ...
            ResultSet resultSet = statement.executeQuery();
            resultSet.next();
            IReport report = new Report(resultSet.getInt("id"), resultSet.getInt("by"), resultSet.getInt("euphaId"), resultSet.getString("reason"));
            connection.close();
            return report;
        } catch (SQLException e) {
            e.printStackTrace();
